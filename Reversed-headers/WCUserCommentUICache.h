//
//     Generated by class-dump 3.5 (64 bit) (Debug version compiled Sep 17 2017 16:24:48).
//
//     class-dump is Copyright (C) 1997-1998, 2000-2001, 2004-2015 by Steve Nygard.
//

#import <objc/NSObject.h>

@class WCUserComment;

@interface WCUserCommentUICache : NSObject
{
    WCUserComment *m_userComment;
    unordered_map_7b5ce4bc m_contentHeight;
    unordered_map_a6c4f618 m_userCommentLayoutStyles;
    unordered_map_7b5ce4bc m_transContentHeight;
    unordered_map_a6c4f618 m_userTransCommentLayoutStyles;
    _Bool _userHasStory;
    _Bool _refHasStory;
    _Bool _isEmotionComment;
}

- (id).cxx_construct;
- (void).cxx_destruct;
@property(nonatomic) _Bool isEmotionComment; // @synthesize isEmotionComment=_isEmotionComment;
@property(nonatomic) _Bool refHasStory; // @synthesize refHasStory=_refHasStory;
@property(nonatomic) _Bool userHasStory; // @synthesize userHasStory=_userHasStory;
@property(retain, nonatomic) WCUserComment *userComment; // @synthesize userComment=m_userComment;
- (void)dealloc;
- (_Bool)isEmpty;
- (unordered_map_a6c4f618 *)userTransCommentLayoutStyles;
- (unordered_map_7b5ce4bc *)transContentHeight;
- (unordered_map_a6c4f618 *)userCommentLayoutStyles;
- (unordered_map_7b5ce4bc *)contentHeight;

@end

