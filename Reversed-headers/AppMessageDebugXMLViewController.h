//
//     Generated by class-dump 3.5 (64 bit) (Debug version compiled Sep 17 2017 16:24:48).
//
//     class-dump is Copyright (C) 1997-1998, 2000-2001, 2004-2015 by Steve Nygard.
//

#import "MMUIViewController.h"

#import "MMRTCMenuResponderDelegate-Protocol.h"

@class CMessageWrap, MMRichTextCoverView, NSString, RichTextView, UIScrollView;

@interface AppMessageDebugXMLViewController : MMUIViewController <MMRTCMenuResponderDelegate>
{
    UIScrollView *m_scrollView;
    RichTextView *m_textLabel;
    CMessageWrap *m_messageWrap;
    MMRichTextCoverView *m_oRichTextCoverView;
}

- (void).cxx_destruct;
- (void)onRichTextViewExit;
- (id)getCurrentViewController;
- (void)onCopy;
- (void)initTextLabel;
- (void)viewDidLoad;
- (id)initWithMessageWrap:(id)arg1;

// Remaining properties
@property(readonly, copy) NSString *debugDescription;
@property(readonly, copy) NSString *description;
@property(readonly) unsigned long long hash;
@property(readonly) Class superclass;

@end

