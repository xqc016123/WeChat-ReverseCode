//
//     Generated by class-dump 3.5 (64 bit) (Debug version compiled Sep 17 2017 16:24:48).
//
//     class-dump is Copyright (C) 1997-1998, 2000-2001, 2004-2015 by Steve Nygard.
//

#import "NSObject-Protocol.h"

@class NSString, NSThread;
@protocol ResourceLoaderDelegate;

@protocol IWXCanvasAdaptorAPI <NSObject>
- (shared_ptr_013fd618)getApp;
- (shared_ptr_bf7a578a)GetDecodeCache;
- (void)CheckAndPost:(function_6d4f7888)arg1;
- (void)SetAppTag:(NSString *)arg1;
- (NSString *)AppTag;
- (struct HolderBase *)GetHolder;
- (void)ReleaseContext;
- (shared_ptr_34631cd5)GetResourceLoader;
- (void)SetResourceLoaderDelegate:(id <ResourceLoaderDelegate>)arg1;
- (void)RemoveCanvas:(int)arg1;
- (shared_ptr_3299e206)GetCanvas:(int)arg1;
- (void)AddCanvas:(shared_ptr_3299e206)arg1;
- (shared_ptr_0ffddd04)CreateCanvasViewDelegate;
- (id)initWithThread:(NSThread *)arg1 context:(struct OpaqueJSContext *)arg2 parent:(struct OpaqueJSValue *)arg3 tag:(NSString *)arg4;
@end

