//
//     Generated by class-dump 3.5 (64 bit) (Debug version compiled Sep 17 2017 16:24:48).
//
//     class-dump is Copyright (C) 1997-1998, 2000-2001, 2004-2015 by Steve Nygard.
//

#import "MMUIButton.h"

@class UIImage, WCDataItem;

@interface WCLikeButton : MMUIButton
{
    WCDataItem *m_item;
    unsigned int m_uiSourceType;
    _Bool m_likeOperating;
    double _iconSize;
    UIImage *_image;
    UIImage *_disabledImage;
    CDUnknownBlockType _likeOperation;
}

- (void).cxx_destruct;
@property(copy, nonatomic) CDUnknownBlockType likeOperation; // @synthesize likeOperation=_likeOperation;
@property(readonly, nonatomic) UIImage *disabledImage; // @synthesize disabledImage=_disabledImage;
@property(readonly, nonatomic) UIImage *image; // @synthesize image=_image;
@property(readonly, nonatomic) double iconSize; // @synthesize iconSize=_iconSize;
@property(nonatomic) unsigned int m_uiSourceType; // @synthesize m_uiSourceType;
@property(retain, nonatomic) WCDataItem *m_item; // @synthesize m_item;
- (void)onLikeFriend;
- (void)LikeBtnReduceEnd;
- (void)LikeBtnReduce;
- (void)LikeBtnEnlarge;
- (id)initWithDataItem:(id)arg1 size:(double)arg2 image:(id)arg3 disabledImage:(id)arg4;
- (id)initWithDataItem:(id)arg1;
- (void)updateLikeBtn;

@end

