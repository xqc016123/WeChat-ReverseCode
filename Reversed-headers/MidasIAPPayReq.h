//
//     Generated by class-dump 3.5 (64 bit) (Debug version compiled Sep 17 2017 16:24:48).
//
//     class-dump is Copyright (C) 1997-1998, 2000-2001, 2004-2015 by Steve Nygard.
//

#import "MidasIAPBaseReq.h"

@class NSString, SKProduct, UIImage, UIViewController;

@interface MidasIAPPayReq : MidasIAPBaseReq
{
    int _productType;
    int _midasUIType;
    NSString *_productId;
    NSString *_saveValue;
    UIImage *_resImg;
    NSString *_resData;
    UIViewController *_contextController;
    double _timeoutLimit;
    long long _grayscaleStrategy;
    SKProduct *_payingProduct;
    NSString *_externalMetaTrackEventId;
}

@property(copy, nonatomic) NSString *externalMetaTrackEventId; // @synthesize externalMetaTrackEventId=_externalMetaTrackEventId;
@property(retain, nonatomic) SKProduct *payingProduct; // @synthesize payingProduct=_payingProduct;
@property(nonatomic) long long grayscaleStrategy; // @synthesize grayscaleStrategy=_grayscaleStrategy;
@property(nonatomic) double timeoutLimit; // @synthesize timeoutLimit=_timeoutLimit;
@property(nonatomic) int midasUIType; // @synthesize midasUIType=_midasUIType;
@property(nonatomic) __weak UIViewController *contextController; // @synthesize contextController=_contextController;
@property(copy, nonatomic) NSString *resData; // @synthesize resData=_resData;
@property(retain, nonatomic) UIImage *resImg; // @synthesize resImg=_resImg;
@property(copy, nonatomic) NSString *saveValue; // @synthesize saveValue=_saveValue;
@property(nonatomic) int productType; // @synthesize productType=_productType;
@property(copy, nonatomic) NSString *productId; // @synthesize productId=_productId;
- (void).cxx_destruct;
- (_Bool)validateParams;
- (id)init;

@end

